...
final class LanguageRepeaterModel {
    ...
    var recentAudioFiles: [URL] {
        guard let directoryURL = try? AudioFileLoader.audioDirectoryURL() else { return [] }
    ...
    func importAudio(from result: Result<[URL], Error>) {
        do {
            ...
            guard let audioURL = selectedURLs.first(where: { AudioFileLoader.isAudioFile($0) }) else { return }
            saveLastOpenedDirectory(for: audioURL)
            let subtitleURL = selectedURLs.first(where: { AudioFileLoader.isSubtitleFile($0) })
    ...
    func loadLastAudioIfNeeded() {
        ...
        if let fileName = UserDefaults.standard.string(forKey: lastAudioFileNameKey),
           let url = try? AudioFileLoader.audioDirectoryURL().appendingPathComponent(fileName),
    ...
    func loadAudio(from url: URL, subtitleURL: URL? = nil) throws {
        ...
        let localURL = try AudioFileLoader.localAudioURL(for: url)
        if let subtitleURL {
            try AudioFileLoader.copySelectedSubtitle(subtitleURL, to: localURL)
        } else {
            try AudioFileLoader.copySidecarSubtitleIfNeeded(from: url, to: localURL)
        }
        ...
        applyLoadedAudioState(for: localURL)
        loadSidecarSubtitlesIfAvailable(for: localURL)

        startWaveformAnalysis(for: localURL)
    }

    private func applyLoadedAudioState(for localURL: URL) {
        ...
        let audioDuration = AudioFileLoader.audioDuration(for: localURL)
    ...
    private func loadSidecarSubtitlesIfAvailable(for localURL: URL) {
        guard let sidecarSubtitles = try? AudioFileLoader.loadSidecarSubtitles(for: localURL),
              !sidecarSubtitles.isEmpty else {
            return
        }
